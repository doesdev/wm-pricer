...
WmPricer.prototype.brickseek = function (opts, cb) {
  ...
  function processBsResults (wmItem, htmlText) {
    ...
    let rows = $('#main').find('.bsapi-inventory-checker-stores').find('tr')
    let item = {
    ...
    item.inventory = rows.map(function () {
      let addrHtml = $(this).find('.store-address').html() || ''
      let priceHtml = $(this).find('.store-price-number').html() || ''
      let stockHtml = $(this).find('.store-quan').html() || ''
      return {
