...
WmPricer.prototype.storeQuery = function (opts) {
  ...
  let processThenPush = () => {
    tmpList.forEach((e) => {
      ...
      let itemId = (e.productId || {}).WWWItemId
      let tmpObj = itemsObj[itemId] = itemsObj[itemId] || {}
      tmpObj.itemId = itemId
  ...
  self.wm.stores.byZip(opts.zip).then((stores) => {
    stores.forEach((s) => {
      let storeCount = 0
      let offset = 0
      function storeSearchCb (count, totalCount) {
        ...
        if (storesSearched === stores.length) processThenPush()
      }
